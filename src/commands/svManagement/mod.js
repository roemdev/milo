    ...
    async execute(interaction) {
        ...
        if (!userRole) {
            const embed = new EmbedBuilder()
                ...
                .setDescription(`${assets.emoji.warn} <@${interaction.user.id}>: No tienes **permiso** para ejecutar este comando.`);
        ...
        if (userData.count >= actionLimit) {
            ...
            const embed = new EmbedBuilder()
                .setColor(assets.color.yellow)
                .setDescription(`${assets.emoji.warn} <@${interaction.user.id}>: Has alcanzado el límite de acciones **(${actionLimit}/${actionLimit})**. Podrás realizar otra acción <t:${timestampReset}:R>.`);
            return interaction.reply({ embeds: [embed], ephemeral: true });
        }
        ...
        if (action === 'ban' && (!memberId || !memberId.bannable)) {
            const embed = new EmbedBuilder()
                ...
                .setDescription(`${assets.emoji.warn} <@${interaction.user.id}>: Hubo un error al **banear** al usuario.`);
            return interaction.reply({ embeds: [embed], ephemeral: true });
        }
        ...
        if (action === 'kick' && (!memberId || !memberId.kickable)) {
            const embed = new EmbedBuilder()
                .setColor(assets.color.red)
                .setDescription(`${assets.emoji.warn} <@${interaction.user.id}>: Hubo un error al **expulsar** al usuario.`);
            return interaction.reply({ embeds: [embed], ephemeral: true });
        }
        ...
        try {
            ...
            const embedDM = new EmbedBuilder()
                ...
                .setDescription(`Has sido **${action === 'ban' ? 'baneado' : 'expulsado'}** del servidor **${interaction.guild.name}**.\nRazón: **${reason}**.`);

            await member.send({ embeds: [embedDM] }).catch(() => {
                const embed = new EmbedBuilder()
                    .setColor(assets.color.yellow)
                    .setDescription(`${assets.emoji.warn} No se pudo enviar el mensaje al usuario.`);
                return interaction.reply({ embeds: [embed], ephemeral: true });
            });
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(`El miembro **${member.tag}** ha sido **${action === 'ban' ? 'baneado' : 'expulsado'}**.\nAcciones restantes: **(${remainingActions}/${actionLimit})**. Reinicio: <t:${timestampReset}:R>.`);
            await interaction.reply({ embeds: [embed] });

            const actionChannel = interaction.guild.channels.cache.get('1284140644843126794');
            if (actionChannel) {
                const channelEmbed = new EmbedBuilder()
                    .setAuthor({ name: user.tag, iconURL: user.displayAvatarURL() })
                    .setTitle(`Miembro ${action === 'ban' ? 'baneado' : 'expulsado'}`)
                    .setColor(assets.color.base)
                    .setDescription(`He ${action === 'ban' ? 'baneado' : 'expulsado'} a **${member.tag}**.\nRazón: **${reason}**.`)
                    .setTimestamp()
            ...
            const embed = new EmbedBuilder()
                .setColor(assets.color.red)
                .setDescription(`${assets.emoji.warn} <@${interaction.user.id}>: Hubo un error al **${action === 'ban' ? 'banear' : 'expulsar'}** al usuario.`);
